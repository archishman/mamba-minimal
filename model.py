...
class MambaBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.groups = args.d_inner
        self.padding = args.d_conv - 1 
    ...
    def ssm(self, x):
        ...
        A = -mlx.exp(self.A_log.float())  # shape (d_in, n)
        D = self.D.float()
    ...
    def selective_scan(self, u, delta, A, B, C, D):
        ...
        deltaA = mlx.exp(einsum(delta, A, 'b l d_in, d_in n -> b l d_in n'))
        deltaB_u = einsum(delta, B, u, 'b l d_in, b l n, b l d_in -> b l d_in n')
        
        # Perform selective scan (see scan_SSM() in The Annotated S4 [2])
        x = mlx.zeros((b, d_in, n), device=deltaA.device)
        ys = []    
        for i in range(l):
            x = deltaA[:, i] * x + deltaB_u[:, i]
            y = einsum(x, C[:, i, :], 'b d_in n, b n -> b d_in')
            ys.append(y)
        y = mlx.stack(ys, dim=1)  # shape (b, l, d_in)
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        output = x * mlx.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps) * self.weight
